...
// Exercice 32, Le loup, la chèvre et le chou
/* Un berger accompagné d'un loup, d'une chèvre et d'un chou, traverse tous les jours une rivière. Le berger ne possède qu'une seule barque. 
La barque ne peut prendre qu'un passager à la fois (plus le berger qui conduit). Sachant qu'en absence du berger, le loup mange la chèvre, et la chèvre mange le chou.
Créez une fonction qui prend un argument de type int, cela correspondra au nombre de traversées de la rivière. 
Votre fonction doit retourner la position du berger, du loup, de la chèvre et du chou suivant la rive où ils se trouvent, 
en prenant compte que ni la chèvre ni le chou ne doivent être mangés.
Exemple :
En entrée:
1
...
En sortie:
Le berger est sur la rive droite.
Le loup est sur la rive gauche.
La chèvre est sur la rive droite.
Le chou est sur la rive gauche.
*/

// Exercice 33, Le problème de Josephus Flavius
/* 
Selon une légende datant du 1er siècle après J-C, Josephus Flavius et ses compatriotes, 
pris au piège dans une grotte par les romains, et préférant la mort à la capture, décidèrent de former un cercle et de tuer la troisième personne vivante rencontrée en suivant le parcours autour du cercle. 
Josephus Flavius ne voulant pas mourir, trouva rapidement la bonne place et fût la seule personne à sortir vivante de la grotte.
Créez une fonction prenant deux arguments de type int, le premier correspondra au nombre de personnes formant le cercle, 
le second sera le nombre de personnes comptées avant l'exécution de la suivante. Votre fonction devra retourner la position initiale de la dernière personne vivante.
*/

/* Exercice 34,
Vous devez mélanger les lettres d'un mot, en gardant la première et dernière lettre.
Les apostrophes, virgules, points etc. ne comptent pas (pour que ça reste facile). Le texte doit être entièrement en minuscule, peut importe l'entrée.
Exemple :
Entrée = "Même si les lettres dans un mot sont dans le désordre on peut lire ce mot"
Sortie = mmêe si les lterets dnas un mot snot dans le désrrdoe on puet lire ce mot"
*/

/* Exercice 35,
Votre fonction prendra 2 arguments :
Un array de mot entièrement en minuscule : exemple : array("bonjour", "test", salut", "bonsoir", "syphopowa")
Une série de lettres espacés d'un espace, en minuscule ou majuscule, une lettre peut être répêté plusieurs fois : Exemple : A e K J D o L A p O
Vous devrez écrire (pas retourner) tous les mots, en allant à la ligne pour chaque mot, qui sont dans l'array entré, qui peuvent se composer à partir des lettres du 2eme argument, en utilisant chaque lettre une seule fois (par mot bien sûr) !
Exemple :
Entrée = <?php fonction(array("bonjour", "test", "salut", "bonsoir", "syphopowa"), "b o O K s L t U A N r I J"); ?>
Sortie = 
bonjour
salut
bonsoir
